...
void setup() {
  ...
  estrellitas = new Estrellita[total];//dimensionar array
  for (int i=0; i<total; i++) {
    estrellitas[i] = new Estrellita();
  }

  img = loadImage("east-java-waterfalls.jpg");

  minim = new Minim(this);
  song = minim.loadFile("Bob Moses - Heaven Only Knows (Official Audio).mp3");
  song.play();
  song.loop(3);
}
